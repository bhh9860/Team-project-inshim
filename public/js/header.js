...
function loginFunc() {
  ...
  axios({
    ...
    if (res.data.result) {
      ...
      sessionStorage.setItem('loggedin_user', JSON.stringify(res.data.loggedin_user));
...
async function checkUserId() {
  ...
  if (!idRegex.test(userId)) {
    alert('아이디는 4~12자리의 영문, 숫자, 대시(-), 밑줄(_)만 사용 가능합니다.');
    userIdInput.focus();
    return false;
  } else {
    axios({
...
async function registFunc() {
  ...
  if (userId === '' || userPw === '' || userName === '' || userCountry === '') {
    ...
    alert('비밀번호는 8~20자리의 영문 대문자, 소문자, 숫자, 특수문자(!@#$%^&*)가 포함되어야 합니다.');
    return false;
...
function checkIfLoggedIn() {
  if (sessionStorage.getItem('loggedin_user')) {
    ...
    axios({
      ...
      url: '/inshim/mypage',
      data: {
        userinfo_id: JSON.parse(sessionStorage.getItem('loggedin_user')).userinfo_id,
      },
    }).then((res) => {
      ...
      document.getElementById('user_country_text').innerText = res.data.user_country;
    });
  } else {
